...
import { ShieldIcon, TrendingUpIcon, TrendingDownIcon, MinusIcon, ChevronsLeft, ChevronLeft, ChevronRight, ChevronsRight, AlertTriangleIcon } from "lucide-react";
...
interface ChallengeRiskTableProps {
    desafios: Desafio[];
    desafioHasExpedicao: DesafioHasExpedicao[];
    expedicoes: Expedicao[];
}

export function ChallengeRiskTable({ desafios, desafioHasExpedicao, expedicoes }: ChallengeRiskTableProps) {
    const [page, setPage] = useState(1);
    const [rowsPerPage, setRowsPerPage] = useState(5);

    // Calcular dados de risco para cada desafio
    const riskData = desafios.map(desafio => {
        ...
        const expedicoesEmAndamento = expedicoesRelacionadas.filter(exp => exp?.status === "Em andamento").length;
        const expedicoesCanceladas = expedicoesRelacionadas.filter(exp => exp?.status === "Cancelada").length;
    ...
    const sortedData = riskData.sort((a, b) => riskOrder[b.nivel_risco] - riskOrder[a.nivel_risco]);
    ...
    const getRiskIcon = (risco: string) => {
        switch (risco) {
            case 'baixo': return <TrendingUpIcon className="w-4 h-4 text-green-600" />;
            case 'medio': return <MinusIcon className="w-4 h-4 text-yellow-600" />;
            case 'alto': return <AlertTriangleIcon className="w-4 h-4 text-orange-600" />;
            case 'critico': return <AlertTriangleIcon className="w-4 h-4 text-red-600" />;
        ...
        <Card className="p-4 gap-2">
            <CardHeader className="p-0">
                <CardTitle className="flex items-center gap-2">
                    ...
                    <span className="text-foreground font-medium">Análise por Nível de Risco</span>
                </CardTitle>
            </CardHeader>
                                    ...
                                    <TableRow key={desafio.id} className="hover:bg-muted/50">
                                        ...
                                        <TableCell>
                                            <div className="flex items-center gap-2 whitespace-nowrap">
                                                <Badge className={getRiskColor(desafio.nivel_risco) + ' text-xs font-medium'}>
                                        ...
                                        <TableCell>
                                            <div className="flex flex-col gap-1 min-w-[120px] max-w-[180px]">
                                                ...
                                                <div className="text-foreground text-xs truncate">
                                                    {desafio.expedicoesEmAndamento} em andamento, {desafio.expedicoesCanceladas} canceladas
                                                </div>
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                            <div className="text-foreground text-xs min-w-[120px] max-w-[180px] truncate">
                                                <div>Em andamento: {desafio.expedicoesEmAndamento}</div>
                                                <div>Canceladas: {desafio.expedicoesCanceladas}</div>
                                            </div>
                                        </TableCell>
                                    </TableRow>
                                ))}
